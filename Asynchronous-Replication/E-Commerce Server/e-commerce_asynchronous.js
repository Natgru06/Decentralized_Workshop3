...
const pool = mysql.createPool({
    ...
    replication: {
        canRetry: true,
        read: [
            { host: 'replica1.example.com', username: 'root', password: 'root' },
            { host: 'replica2.example.com', username: 'root', password: 'root' }
        ],
        write: { host: 'master.example.com', username: 'root', password: 'root' }
    }
});
...
app.get('/products', (req, res) => {
    ...
    pool.query(sql, filterValues, (err, results) => {
        if (err) {
...
app.put('/products/:id', (req, res) => {
    ...
    pool.query(sql, [updatedProduct, productId], (err, result) => {
        if (err) {
...
app.post('/cart/:userId', (req, res) => {
    ...
    pool.query(sql, [userId, productId, quantity], (err, result) => {
        if (err) {
        ...
        pool.query('SELECT * FROM shopping_carts WHERE user_id = ?', [userId], (err, results) => {
            if (err) {
